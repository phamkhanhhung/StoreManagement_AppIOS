...
class TabBarVC: UITabBarController {
    override func viewDidLoad() {
        ...
        let nav2 = generateNavController(vc: ProfileHomeVC(), title: "Me", image: #imageLiteral(resourceName: "tab_profile"), selectedImage: #imageLiteral(resourceName: "tab_profile_sl"))
...
extension TabBarVC: UITabBarControllerDelegate {
    func tabBarController(_ tabBarController: UITabBarController, shouldSelect viewController: UIViewController) -> Bool {
        ...
        if let nav = viewController as? UINavigationController {
            if let vc = nav.viewControllers.last, vc is OrderHistoryVC {
                if !isLogin {
